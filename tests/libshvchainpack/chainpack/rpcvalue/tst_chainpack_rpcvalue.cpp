...
private:
	...
	void binaryTest()
	{
		...
		/*
		for (int i = ChainPack::TypeInfo::Null_Array; i <= ChainPack::TypeInfo::DateTimeTZ_Array; ++i) {
			RpcValue::Blob out;
			out += i;
			ChainPack::TypeInfo::Enum e = (ChainPack::TypeInfo::Enum)i;
			std::ostringstream str;
		...
		*/
		...
		}
		{
			...
			for (RpcValue::UInt n = 0; n < 64; ++n) {
				RpcValue cp1{n};
				std::stringstream out;
				ChainPackWriter wr(out); wr << cp1; size_t len = out.str().size();
				if(n < 10)
					qDebug() << n << " " << cp1.toCpon() << " len: " << len << " dump: " << binary_dump(out.str()).c_str();
				QVERIFY(len == 1);
				ChainPackReader rd(out); RpcValue cp2; rd >> cp2;
				QVERIFY(cp1.type() == cp2.type());
