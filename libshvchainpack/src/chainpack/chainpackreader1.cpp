...
void ChainPackReader1::read(RpcValue::MetaData &meta_data)
{
	RpcValue::IMap imap;
	RpcValue::Map smap;
...
void ChainPackReader1::read(RpcValue &val)
{
...
uint64_t ChainPackReader1::readUIntData(std::istream &data, bool *ok)
{
	int bitlen;
...
RpcValue ChainPackReader1::readData(ChainPack::PackingSchema::Enum type_info, bool is_array)
{
	RpcValue ret;
	if(is_array) {
		RpcValue::List val = readData_Array(type_info);
		ret = RpcValue(val);
	}
	else {
		switch (type_info) {
...
RpcValue::List ChainPackReader1::readData_List()
{
	RpcValue::List lst;
	while(true) {
		int b = m_in.peek();
...
RpcValue::Map ChainPackReader1::readData_Map()
{
	RpcValue::Map ret;
	while(true) {
		int b = m_in.peek();
...
RpcValue::IMap ChainPackReader1::readData_IMap()
{
	RpcValue::IMap ret;
	while(true) {
		int b = m_in.peek();
...
/*
static RpcValue::Type typeInfoToArrayType(ChainPack::PackingSchema::Enum type_info)
{
	switch (type_info) {
...
*/
RpcValue::List ChainPackReader1::readData_Array(ChainPack::PackingSchema::Enum array_type_info)
{
	//RpcValue::Type type = typeInfoToArrayType(array_type_info);
	RpcValue::List ret;
