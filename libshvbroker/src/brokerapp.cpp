...
chainpack::AclRolePaths BrokerApp::aclRolePaths(const std::string &role_name)
{
	...
	for(const auto &kv : paths.toMap()) {
		...
		auto fwd_role = m.value("forwardGrant").toString();
		if(!fwd_role.empty()) {
			role = fwd_role;
			grant.notResolved = true;
		}
		if(!role.empty()) {
			...
			shvError() << "unsupported ACL path definition";
		}
		ret[path] = grant;
	}
	return ret;
...
cp::AccessGrant BrokerApp::accessGrantForRequest(rpc::CommonRpcClientHandle *conn, const std::string &rq_shv_path, const shv::chainpack::RpcValue &rq_grant)
{
	...
	const std::vector<std::string> &user_flattent_grants = is_request_from_master_broker
			? std::vector<std::string>{rq_grant.toString()} // master broker has allways grant masterBroker
